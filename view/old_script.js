	...
	$(grid).jqGrid({
		...
		onSort: function(id) {
			...
			var row = $(this) .jqGrid('getGridParam', 'selrow');
			 nofaktur = $('#grid').jqGrid('getCell', row, 'id')
			// nofaktur = $(this)
	...
	$(grid).navButtonAdd(pager, {
		...
		onClickButton:function(){
			if ($(grid).jqGrid('getGridParam','selrow') !== null) {
				...
				activeGrid = undefined
				nofaktur = $(grid).jqGrid('getGridParam','selrow')
				confirmDeleteCustomer(nofaktur)
			} else {
				alert('Please, select row')
			}
		},
	...
	$(grid).navButtonAdd(pager, {
   	caption: "Report",
		...
		onClickButton:function(){
				...
				.dialog({
					...
					buttons: {
						'Report': function() {
							...
							window.open( baseUrl + `TRANSAKSI/report?${params}&start=${start}&limit=${limit}&orders_sidx=${ordersPostData.sidx}&orders_sord=${ordersPostData.sord}`)
						},
						'Cancel': function() {
							activeGrid = '#grid'
							$(this).dialog('close')
			...
			// window.open('TRANSAKSI/report/' + nofaktur)
		},
	})
	*/
	/*
...
function addCustomer() {
	$(tableDialog).load('create-modal.php', function() {
		$($(tableForm + ' input')[0]).focus()
	...
	$.ajax({
		...
		success: function(res) {
			if (res.status == 'submitted') {
				...
				// loadgrid();
				// $.ajax({
				// 	url: 'ajax.php?cari=show&' + res.postData.no_invoice + '/' + res.postData[$(TRANSAKSITable).jqGrid('getGridParam', 'sortname')] + '/' + $(TRANSAKSITable).jqGrid('getGridParam', 'sortname') + '/' + $(TRANSAKSITable).jqGrid('getGridParam', 'sortorder') + '/' + $(grid).jqGrid('getGridParam', 'postData').rows,
				// 	type: 'GET',
				// 	dataType: 'JSON',
				// 	success: function(res) {
				// 		if (res.data) {
	...
	$.ajax({
		...
		success: function(res) {
			...
			if (res.status == 'submitted') {
				...
				$.ajax({
					url: baseUrl + '/TRANSAKSI/show/' + res.postData.no_invoice + '/' + res.postData[$(grid).jqGrid('getGridParam', 'sortname')] + '/' + $(grid).jqGrid('getGridParam', 'sortname') + '/' + $(grid).jqGrid('getGridParam', 'sortorder') + '/' + $(grid).jqGrid('getGridParam', 'postData').rows,
					type: 'GET',
					dataType: 'JSON',
					success: function(res) {
						if (res.data) {
						...
						selectedPage = res.page
						setTimeout(function() {
							$('#grid').trigger('reloadGrid', [{page: selectedPage}])
						}, 50)
					}
				})
			} else {
...
function editCustomer(nofaktur) {
	$(tableDialog).load('update-modal.php?id='+nofaktur, function() {
		$($(tableForm + ' input')[0]).focus()
...
function updateCustomer(nofaktur) {
	$.ajax({
...
function confirmDeleteCustomer(nofaktur) {
	$(tableDialog)
		.load('delete-modal.php?id=' + nofaktur)
		.dialog({
		modal: true,
	...
	function deleteCustomer(nofaktur) {
		$.ajax({
	...
	$(orderTable).jqGrid({
   	...
   	rowList:[10,20,30],
		...
		onSelectRow: function(id) {
			activeGrid = '#order'
			nofaktur = $(this).jqGrid('getRowData', id).no_invoice
		},
		keys: true,
	})
            ...
            if (
              ...
              if (35 === e.keyCode) {
              	...
              	if (currentPage !== lastPage) {
              		...
              		if (e.ctrlKey) {
	              		if ($(s).jqGrid('getGridParam', 'selrow') !== $('#grid').find(">tbody>tr.jqgrow").filter(":last").attr('id')) {
              ...
              if (36 === e.keyCode) {
              	triggerClick = true
              	if (e.ctrlKey) {
              		if ($(s).jqGrid('getGridParam', 'selrow') !== $('#grid').find(">tbody>tr.jqgrow").filter(":first").attr('id')) {
