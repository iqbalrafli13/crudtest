...
rowList:[10,20,30],
    ...
    loadComplete: function() {
        ...
        setTimeout(function() {
            $('#grid tbody tr td:not([aria-describedby=customer_rn])').highlight(highlightSearch)

            if (indexRow > $('#grid').getDataIDs().length - 1) {
                indexRow = $('#grid').getDataIDs().length - 1
            }

            if (triggerClick) {
                $('#' + $('#grid').getDataIDs()[indexRow]).click()
                triggerClick = false
            } else {
                $('#grid').setSelection($('#grid').getDataIDs()[indexRow])
            }
